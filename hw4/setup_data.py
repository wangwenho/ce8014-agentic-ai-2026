...
def init_db(conn):
    ...
    cursor.execute(
        """CREATE TABLE regulations (reg_id INTEGER PRIMARY KEY, name TEXT NOT NULL, category TEXT)"""
    )
    cursor.execute(
        """CREATE TABLE articles (art_id INTEGER PRIMARY KEY AUTOINCREMENT, reg_id INTEGER, article_number TEXT, content TEXT, FOREIGN KEY(reg_id) REFERENCES regulations(reg_id))"""
    )
    conn.commit()
    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            if parser_mode == "article":
                pattern = re.compile(
                    r"^\s*Article\s+([0-9]+(?:\-[0-9]+)?)", re.IGNORECASE
                )
            ...
            for line in lines:
                ...
                if (
                    ...
                    or (line.isdigit() and len(line) < 4)
                ):
                    continue
...
def main():
    ...
    print("🚀 Starting ETL (Refactored for Clean Data)...")
    reg_id_counter = 1
    total = 0
    ...
    for filename, reg_name, category, p_mode, use_layout in PDF_CONFIG:
        ...
        cursor.execute(
            ...
            (reg_id_counter, reg_name, category),
        )
